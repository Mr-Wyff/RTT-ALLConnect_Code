...
#define AT_SEND_CMD(client, resp, cmd)                                     \
    do {                                                                   \
        (resp) = at_resp_set_info((resp), 1024, 0, 5 * RT_TICK_PER_SECOND); \
...
extern int onenet_upload_example(void);
...
#define INIT_RETRY    5
    ...
    while (retry_num--)
    {
        ...
        AT_SEND_CMD(client, resp, "AT+CIFSR");

        //AT_SEND_CMD(client, resp, "AT+CIPMODE=1");
